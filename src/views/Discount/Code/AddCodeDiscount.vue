<template>
    <div class="body-page add-code1 add-code" id="body-page">
        <div class="body">
            <div class="card">
                                ...
                                <form class="form-input">
                                    <div class="form-row">
                                        <div class="my-input col-lg-12 row">
                                            ...
                                            <b-form-input v-model="code.name" class="col-lg-7 col-md-7 col-sm-11 col-11 input-field" name="name">
                                            </b-form-input>
                                            <v-tooltip color="error" right v-if="v$.code.name.$error">
                                                <template v-slot:activator="{ on, attrs }">
                                                    <v-icon color="red" dark v-bind="attrs" v-on="on">
                                                        mdi-exclamation
                                                    </v-icon>
                                                </template>
                                                <span>{{ v$.code.name.$errors[0].$message }}</span>
                                            </v-tooltip>
                                        </div>
                                    </div>
                                    ...
                                    <div class="form-row">
                                        <div class="my-input col-lg-12 row" style="margin-top: -12px">
                                            ...
                                            <b-form-input type="number" v-model="code.value" class="col-lg-7 col-md-7 col-sm-11 col-11 input-field" name="value">
                                            </b-form-input>
                                            <v-tooltip color="error" right v-if="v$.code.value.$error">
                                                <template v-slot:activator="{ on, attrs }">
                                                    <v-icon color="red" dark v-bind="attrs" v-on="on">
                                                        mdi-exclamation
                                                    </v-icon>
                                                </template>
                                                <span>{{ v$.code.value.$errors[0].$message }}</span>
                                            </v-tooltip>
                                        </div>
...
export default {
    ...
    methods: {
        submitForm() {
            ...
            if (!this.v$.$error){
                this.sendData();
                this.$router.replace({ name: 'add-code2' })
            }
            console.log( this.$store.state.code.name)
            console.log( this.$store.state.code.value)
        },
    },
};
</script>
